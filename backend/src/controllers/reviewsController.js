// CONTROLLER TO FETCH REAL REVIEWS FROM MONGODB
let Review;
try {
...
const getRatingStats = async (req, res) => {
  try {
    console.log('Getting rating stats from MongoDB...');
    ...
    // Get all reviews for this astrologer from MongoDB
    const reviews = await Review.find({ astrologerId: astrologerId });
    console.log('Found reviews in MongoDB:', reviews.length);
    ...
    console.log('Calculated stats from MongoDB:', result);
...
const getReviews = async (req, res) => {
  try {
    console.log('Getting reviews from MongoDB...');
    ...
    // Get all reviews for this astrologer from MongoDB, sorted by newest first
    const reviews = await Review.find({ astrologerId: astrologerId })
      .sort({ createdAt: -1 })
    ...
    console.log('Transformed reviews for frontend:', transformedReviews.length);
